...
class Grid():
    def __init__(self, rank, grid_dim, grid_left, grid_right,
        ...
        self.data = np.zeros((self.cell_edges_x.size+3, self.cell_edges_y.size+3),
                             dtype='double')
        self.shape = self.data.shape

    def _create_boundary_arrays(self): 
        # Copy the edges of the active zone  
        self.l_edge  = self.data[ 2:-2, 2:4].copy() # x is second index  
        self.r_edge  = self.data[ 2:-2,-3:-1].copy()
        self.u_edge  = self.data[ 2:4 , 2:-2].copy()
        self.d_edge  = self.data[-3:-1, 2:-2].copy()

        # Create empty arrays to hold edges from other active zones
        # The corners don't matter
        self.l_ghost = np.zeros((self.data.shape[0]-4, 2), dtype='double') 
        self.r_ghost = np.zeros((self.data.shape[0]-4, 2), dtype='double')
        self.u_ghost = np.zeros((self.data.shape[1]-4, 2), dtype='double')
        self.d_ghost = np.zeros((self.data.shape[1]-4, 2), dtype='double')
    ...
    def update_boundaries(self):
        ...
        self.data[ 2:-2, 0:2] = self.l_ghost.copy()
        self.data[ 2:-2,-2: ] = self.r_ghost.copy()
        self.data[ 0:2 , 2:-2] = self.u_ghost.T.copy()
        self.data[-2:  , 2:-2] = self.d_ghost.T.copy()
...
if __name__ == "__main__":
    ...
    source_term[2:-2,2:-2] = charge_density(xx,yy).T
    ...
    for k in range(5):
        ...
        for i in range(2, my_grid.shape[0]-2):
            for j in range(2, my_grid.shape[1]-2):
                new[i,j] = factor*(
        ...
        my_grid.data[2:-2,2:-2] = new[2:-2,2:-2].copy()      
        
    plt.pcolormesh(my_grid.cell_edges_x, my_grid.cell_edges_y,
                   my_grid.data[2:-2,2:-2].T)   
    plt.show()
